    ...
    <div v-if="field.render == 'buttons' && field.buttons">
        <template v-for="(btn, idx) in field.buttons" :key="idx">
            <template v-if="btn.display ? btn.display(row, field) : true">
                ...
                <el-tooltip
                    v-if="btn.render == 'tipButtonn' && ((btn.name == 'abcde' && baTable.auth('abcde')) || btn.name != 'abcde')"
                    :disabled="btn.title && !btn.disabledTip ? false : true"
                    :content="btn.title ? t(btn.title) : ''"
                    placement="top"
                >
...
import {pstUserpsw}from '/@/api/backend/user/group'
...
const emit = defineEmits(['claickchi'])
const clickpws=(btn)=>{
    console.log(btn)
    ...
    .then(() => {
        pstUserpsw(btn.id).then(res=>{
            emit('claickchi',res.data)
        })
