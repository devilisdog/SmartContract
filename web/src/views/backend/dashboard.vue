    ...
    <div class="default-main">
            ...
            <el-row :gutter="20">
                <el-col :sm="12" :lg="6">
                    <div class="small-panel user-reg suspension">
                        <div class="small-panel-title">店铺数量</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#8595F4" size="20" name="fa fa-user" />
                                <span id="user_reg_number">{{state.Statistical.shopCount.value}}</span>
                            </div>
                            <div class="content-right">{{state.Statistical.shopCount.newUserPercent}}%</div>
                        </div>
                    </div>
                </el-col>
                <el-col :sm="12" :lg="6">
                    <div class="small-panel file suspension">
                        <div class="small-panel-title">用户数量</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#AD85F4" size="20" name="fa fa-user" />
                                <span id="file_number">{{state.Statistical.userCount.value}}</span>
                            </div>
                            <div class="content-right">{{state.Statistical.userCount.newUserPercent}}%</div>
                        </div>
                    </div>
                </el-col>
                ...
                <el-col :sm="12" :lg="6">
                    <div class="small-panel user-reg suspension">
                        <div class="small-panel-title">店铺存续</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#8595F4" size="20" name="fa fa-file-text" />
                                <span id="user_reg_number">{{state.Statistical.ShopAllMoney.value}}</span>
                            </div>
                            <div class="content-right">{{state.Statistical.ShopAllMoney.newUserPercent}}%</div>
                        </div>
                    </div>
                </el-col>
                <el-col :sm="12" :lg="6">
                    <div class="small-panel file suspension">
                        <div class="small-panel-title">今日流水</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#AD85F4" size="20" name="fa fa-line-chart" />
                                <span id="file_number">{{state.Statistical.orderCompleteCount.value}}</span>
                            </div>
                            <div class="content-right">{{state.Statistical.orderCompleteCount.newUserPercent}}%</div>
                        </div>
                    </div>
                </el-col>
            ...
            <el-row :gutter="20">
                <el-col :sm="12" :lg="6">
                    <div class="small-panel users suspension">
                        <div class="small-panel-title">待处理收款渠道</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#FF0000" size="20" name="fa fa-exclamation-triangle" />
                                <span id="users_number">{{state.WorkData.PaymentChannelsReviewed.value}}</span>
                            </div>
                            <div class="content-right">去处理</div>
                        </div>
                    </div>
                </el-col>
                <el-col :sm="12" :lg="6">
                    <div class="small-panel addons suspension">
                        <div class="small-panel-title">待处理店铺认证</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#FF0000" size="20" name="fa fa-exclamation-triangle" />
                                <span id="addons_number">{{state.WorkData.PendingStoreAuthentication.value}}</span>
                            </div>
                            <div class="content-right">去处理</div>
                        </div>
                    </div>
                ...
                <el-col :sm="12" :lg="6">
                    <div class="small-panel users suspension">
                        <div class="small-panel-title">待处理提现管理</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#FF0000" size="20" name="fa fa-exclamation-triangle" />
                                <span id="users_number">{{state.WorkData.PendingWithdrawalRequests.value}}</span>
                            </div>
                            <div class="content-right">去处理</div>
                        </div>
                    </div>
                ...
                <el-col :sm="12" :lg="6">
                    <div class="small-panel addons suspension">
                        <div class="small-panel-title">待处理投诉建议</div>
                        <div class="small-panel-content">
                            <div class="content-left">
                                <Icon color="#FF0000" size="20" name="fa fa-exclamation-triangle" />
                                <span id="addons_number">{{state.WorkData.PendingFeedBack.value}}</span>
                            </div>
                            <div class="content-right">去处理</div>
                        </div>
                    </div>
        ...
        <div class="growth-chart">
            <el-row :gutter="20">
                ...
                <el-col class="lg-mb-20" :xs="24" :sm="24" :md="12" :lg="9">
                    <el-card shadow="hover" :header="监控系统信息">
                        <div class="user-growth-chart" :ref="chartRefs.set"></div>
                    </el-card>
                </el-col>
...
const state: {
    ...
    workingTimeFormat: '',
    pauseWork: false,
    titlelist:[ "CPU", "内存", "/", "/home" ],
    percentlist:[ 0, 0, 0, 0 ],
})

index().then((res) => {
    //console.log(1,state,res)
    state.remark = res.data.remark
    state.WorkData = res.data.WorkData
    state.Statistical = res.data.Statistical
    state.SysData = res.data.SysData
    state.titlelist =res.data.SysData.title
    state.percentlist =res.data.SysData.percent
...
const initSysDataChart = () => {
    
   
    
    const SysDataChart = echarts.init(chartRefs.value[1] as HTMLElement)
    const option = {
        grid: {
            left: 15, // 设置左边距为 0
            right: 15, // 设置右边距为 0
            top: 60, // 设置上边距为 0
            bottom: 10, // 设置下边距为 0
            containLabel: true, // 根据图表内容自动计算边距
            show: false  // 设置为 false，隐藏背景网格
        },
        xAxis: {
        ...
        series: [
            {
                data:  state.percentlist,
                type: 'bar',
                barWidth: '15%', // 柱宽度设置为100%
                barCategoryGap: '0', // 柱间距设置为0%   
                barGap: '0', // 柱组间距设置为0%
                showBackground:true,
                color: '#4671ff',
                backgroundStyle: {
    ...
    SysDataChart.setOption(option)
    state.charts.push(SysDataChart)
}

const initFileGrowthChart = () => {
    const fileGrowthChart = echarts.init(chartRefs.value[2] as HTMLElement)
    const option = {
        grid: {
            top: 30,
            right: 0,
...
const initUserSourceChart = () => {
    const UserSourceChart = echarts.init(chartRefs.value[3] as HTMLElement)
    const pathSymbols = {
        reindeer:
...
const initUserSurnameChart = () => {
    const userSurnameChart = echarts.init(chartRefs.value[4] as HTMLElement)
...
onMounted(() => {
    startWork() //工作相关
    //initCountUp()
    initUserGrowthChart() //会员增长情况
    
    initSysDataChart() //会员增长情况
    //initFileGrowthChart() //附件情况
    //initUserSourceChart() // 用户来源
    //initUserSurnameChart() //用户姓氏分布
