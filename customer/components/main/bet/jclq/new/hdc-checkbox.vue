		...
		<view class="checkbox">
			<view class="box">
				<view v-if="noshowgame(1,'rf')" class="check" :style="styledata.value.rf.style[index]==0?'':' background-color: red; color: #fff;'"
					v-for="item,index in styledata.value.rf.res" @click="change('rf',index,0) "
					>
					<view class="huadan" style="display: flex;flex-direction: column;">
						<view class='inner-box1' v-show="noshowdan(1,item)">
							<text style="transform: scale(0.8);">单</text>
						</view>
						<view style="display: flex;flex-direction: row;">
							<text style="font-weight: 700;font-size:28rpx;">{{item}}
								<text v-show="index==1" :style="styledata.value.rf.style[index]!=0?'color:#ffffff':props.fixSelectData.rf.goalLine<0?'color:green':'color:red'">({{props.fixSelectData.rf.goalLine}}{{}})
								</text>
