		...
		<view class="checkbox">
			<view class="box">
				<view v-if="noshowgame('HDC')" class="check" :style="styledata.value.rf.style[index]==0?'':' background-color: red; color: #fff;'"
					v-for="item,index in styledata.value.rf.res" @click="props.fixSelectData['HDC'][styledata.value.rf.keyArr[index]]==0?remindchange():change('rf',index,'HDC') "
					>
					<view class="huadan" style="display: flex;flex-direction: column;">
						<view class='inner-box1' v-show="noshowdan('HDC')">
...
<script setup>
	...
	const change=(game,index,gameId)=>{
		...
		if(styledata.value[game].style[index]==0){//添加
			...
			if(game=='rq'){temporaryArr.value.gameData['goal']=props.fixSelectData['HDC'].GoalLine}//如果是让球玩法添加让球数值
