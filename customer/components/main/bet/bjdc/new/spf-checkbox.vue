		...
		<view class="checkbox ">
			...
			<view class="box">
				...
				<view class="box" v-if="noshowgame()">
					<view class="check" v-for="item,index in styledata.value.had.res" :key="index"
					:style="styledata.value.had.style[index]==0?'':' background-color: red; color: #fff;'" 
					@click="change('had',index)">
						<view class="huadan">
							<text style="font-weight: 700;font-size:28rpx;">{{item}}</text>
							<text style="margin-left: 5px;font-size:26rpx;">
								{{props.fixSelectData.odds.spf[styledata.value.had.rate[index]]}}
							</text>
						</view>
					</view>
				</view>
...
<script setup>
	...
	const change=(game,gameIndex)=>{
		var array = {
			...
			'data': [{
				'res': styledata.value["had"].res[gameIndex],
				'rate':props.fixSelectData.odds.spf[styledata.value["had"].rate[gameIndex]],
				'poolEs':1//判断是否单关
			}],
		}
		if(styledata.value["had"].style[gameIndex]==0){//添加
			...
			if(props.fixSelectData.odds.spf['goal']!='0'){//如果有让球玩法添加让球数值
				temporaryArr.value.gameData['goal']=props.fixSelectData.odds.spf['goal']
			}
			styledata.value["had"].style[gameIndex] = 1
			emits('getSelectList',temporaryArr.value)//最后把处理好的数据发给父页面
		}else{//删除
			styledata.value["had"].style[gameIndex] = 0
			if(temporaryArr.value.gameData.data.length==1){//当只有一个的情况下直接全部删除
				...
				var Delindex = temporaryArr.value.gameData.data.findIndex(item =>item.res == styledata.value["had"].res[gameIndex])
