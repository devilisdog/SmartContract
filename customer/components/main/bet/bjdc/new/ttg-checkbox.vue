		...
		<view class="checkbox ">
			<view class="box">
				...
				<view class="box" v-if="noshowgame()" style="display: flex;flex-wrap: wrap;">
					...
					@click="change(index)" style="margin-top: 5rpx; flex:25%">
						<view class="huadan" style="flex-direction: column;">
							...
							<text style="margin-left: 5px;font-size: 26rpx;">
								{{props.fixSelectData.odds.jq[styledata.value.rate[index]] == null ? '0.00' : props.fixSelectData.odds.jq[styledata.value.rate[index]]}}
							</text>
						</view>
					</view>
				</view>
	...
	const change=(gameIndex)=>{
		var array = {
			...
			'data': [{
				...
				'rate':props.fixSelectData.odds.jq[styledata.value.rate[gameIndex]],
				'poolEs':1//判断是否单关
			}],
		}
