	...
	<view style="border-radius:20rpx 20rpx 0 0;background-color:#fff;">
		...
		<view class="suspensionbox" style="">
			<view class="boxt" style="flex:1.5;">
				<text style="font-size: 40rpx; font-weight: 600;">{{props.fixSelectData.HostFull}}</text>
			</view>
			<view class="boxt" style="flex:0.3;">
				<text style="color: #ff0000; font-size: 40rpx; font-weight: 600;">VS</text>
			</view>
			<view class="boxt" style="flex:1.5;">
				<text style="font-size: 40rpx; font-weight: 600; ">{{props.fixSelectData.GuestFull}}</text>
			</view>
		</view>
		<view class="gamebox" style="margin: 15rpx;">
			<view class="box" style="flex-wrap: wrap;">
				<view class="check" v-for="item,index in props.styledata.value.res"
					...
					@click="props.fixSelectData[props.styledata.value.rate[index]]==0?remindchange():change(index)">
					<view class="huadan">
						<text style="font-weight: 700;">{{item}}</text>
						<view style="display: flex;flex-direction: row;">
							<text style="margin-left: 5px; font-weight: 700;">
								{{props.fixSelectData[props.styledata.value.rate[index]]==0?'--':props.fixSelectData[props.styledata.value.rate[index]]}}
							</text>
						</view>
					</view>
				</view>
...
<script setup>
	...
	const emits = defineEmits(['closeupup', 'getfixSelectData','getfixSelectDataS','remind'])

	const scrollTop = ref(0)
	const change = (index) => {
		emits('getfixSelectData',index)
	}
	const cancel = () => {
		emits('closeupup')
	}
