		...
		<view class="checkbox ">
			...
			<view class="box">
				<view class="box" >
					...
					@click="props.fixSelectData[styledata.value.had.rate[index]]==0?remindchange():change('had',index)">
						<view class="huadan">
							<text style="font-weight: 700;font-size:28rpx;">{{item}}</text>
							<text style="margin-left: 5px;font-size:26rpx;">
								{{props.fixSelectData[styledata.value.had.rate[index]]==0?'--':props.fixSelectData[styledata.value.had.rate[index]]}}
							</text>
						</view>
					</view>
				</view>
	...
	const change=(game,gameIndex)=>{
		var array = {
			...
			'data': [{
				...
				'rate':props.fixSelectData[styledata.value["had"].rate[gameIndex]],
				'poolEs':1//判断是否单关
			}],
		}
