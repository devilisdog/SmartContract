<template>
	...
	<view
		...
		<view style="display: flex; justify-content: flex-start;align-items: center;flex-wrap:wrap; height: 190rpx;">
			<view v-for="itme,index in 9" @click="dianClick(itme)" class="baiwei"
				...
				<view v-show="queStlye[index] == 2" style="margin: auto;">
					<view
						...
						<text style="font-size: 32rpx;">{{itme}}</text>
						<text>胆</text>
					</view>
				</view>
			</view>
...
<script setup>	
	...
	// console.log(e);
		if (queStlye[e-1] == 0) {
			data.value.data.data[0].push(e)
			isnote()
			queStlye[e-1] = 1
			return
		}
		if (queStlye[e-1] == 1) {
			var index = data.value.data.data[0].findIndex(item =>
				//console.log(item),
				item == e // 11  11
			)
			// 删除掉里面的投注内容
			data.value.data.data[0].splice(index, 1)
			isnote()
			queStlye[e-1] = 0
			return
		}
	...
	const mathbum = (min, max, count) => {
		...
		for (var i = 0; i <= numlist.length - 1; i++) {
			...
			queStlye[numlist[i]-1] = 1
			data.value.data.data[0].push(numlist[i])
		}
		isnote()
	}
	// 跨度注数字典
	const zidian =reactive([0,18,16,14,12,10,8,6,4,2])
	const isnote =()=>{
		var note=0
		for(var i=0;i<=data.value.data.data[0].length-1;i++){
			// 123
			note+=zidian[data.value.data.data[0][i]]
		}
		data.value.note = note
		data.value.money = data.value.note*2
		data.value.data.data[0] = common.quickSort(data.value.data.data[0])
		Emits('datachange',data.value,'zu3_kuadufushi')
	}
