<template>
	<view style="width: 700rpx;margin:auto;display:flex;justify-content: space-between;margin-top: -30rpx;">
			<view style="color: #9c9c9c;">
				<text>至少选择<text style="color: red;">2</text>个号码</text>
			</view>
	</view>
	<!-- 直选复式-球 -->
...
<script setup>	
	...
	const isnote =()=>{
		...
		if(data.value.data[0].length>=2){
			data.value.note = common.TakeMN(data.value.data[0].length, 2)*10
			data.value.money = data.value.note * 2
			Emits('datachange', data.value, 'zuxuan_quanbao')
		}else{
			data.value.note = 0
			data.value.money = data.value.note * 2
			Emits('datachange', data.value, 'zuxuan_quanbao')
