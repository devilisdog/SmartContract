<template>
	<view style="width: 700rpx;margin:auto;display:flex;justify-content: space-between;margin-top: -30rpx;">
		<view style="color: #9c9c9c;">
			<text>至少选择<text style="color: red;">3</text>个号码(双击设胆)，奖金</text>
			<text style="color:red;">173</text>
			<text>元</text>
		</view>
	</view>
	<!-- 直选复式-球 -->
	<view style="display: flex;width: 700rpx;justify-content: flex-end;position: relative;top: -42rpx;left: 26rpx;">
		<view style="width:90rpx; height: 40rpx;text-align: center;" class="borderuu" @click="mathbum(0,10,3)">
...
<script setup>
	...
	const dianClick = (e) => { //球的状态判断
		if (queStlye[e] == 0) {
			...
			data.value.note = common.TakeMN(data.value.data[0].length, 3)
			data.value.money = data.value.note * 2
			data.value.data[0] = common.quickSort(data.value.data[0])
			Emits('datachange', data.value, 'zu6_fushi')
			queStlye[e] = 1
			return
		}
		if (queStlye[e] == 1) {
