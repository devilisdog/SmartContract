					...
					@click="change(index)">
						<view class="huadan">
							...
							<text style="margin-left: 5px;font-size:26rpx;">
								{{props.fixSelectData.odds.sf[styledata.value.rate[index]]}}
							</text>
						</view>
					</view>
				</view>
...
<script setup>
	...
	const change=(index)=>{
		var array = {
			...
			'data': [{
				...
				'rate':props.fixSelectData.odds.sf[styledata.value.rate[index]],
			}],
		}
		if(styledata.value.style[index]==0){//添加
			...
			if(props.fixSelectData.odds.sf['goal']!='0'){//如果有让球玩法添加让球数值
				temporaryArr.value.gameData['goal']=props.fixSelectData.odds.sf['goal']
			}
	...
	const noshowgame=()=>{
		if(props.fixSelectData.sell_status=='1'){
			return true
		}else{
			return false
		}
