	...
	<view class="yc-data-checkbox">
		<view class="box">
			<view class="box" v-if="noshowgame('TTG')" style="flex-wrap: wrap;">
				<view class="check" v-for="item,index in styledata.value.jq.res" :key="index"
					style="height: 75rpx;flex: 25%;"
					:style="styledata.value.jq.style[index]==0?'':' background-color: red; color: #fff;'"
					@click="props.fixSelectData['TTG'][styledata.value.jq.keyArr[index]]==0?remindchange():change('jq',index,'TTG')">
					<view class="huadan">
