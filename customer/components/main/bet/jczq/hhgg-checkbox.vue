		...
		<view class="checkbox">
			...
			<view class="box"><!-- 让球胜平负 -->
				<view class="box" v-if="noshowgame(1,'rq')">
					...
					:style="styledata.value.rq.style[index]==0?'':' background-color: red; color: #fff;'" @click="change('rq',index)">
						<view class="huadan">
							<view :class="noshowdan(1)?'inner-box1':''">
								<text v-show="noshowdan(1)" style="transform: scale(0.8);">单</text>
							</view>
							<view style="display: flex;flex-direction: row;">
								<text style="font-weight: bold;align-self:center;font-size:28rpx;">{{item}}</text>
								<text style="margin-left: 5px;align-self:center;font-size:26rpx;">
	...
	const init=()=>{
		...
		var arr = {
			...
			bf:{
				...
				res:['1:0','2:0','2:1','3:0','3:1','3:2','4:0','4:1','4:2','5:0','5:1','5:2','胜其它','0:0','1:1','2:2','3:3','平其它','0:1','0:2','1:2','0:3','1:3','2:3','0:4','1:4','2:4','0:5','1:5','2:5','负其它'],
			}
		}
		styledata.value = arr
	}
