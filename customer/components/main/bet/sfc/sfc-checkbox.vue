		...
		<view class="checkbox ">
			...
			<view class="box">
				...
				<view class="box">
					...
					:style="styledata.value.style[index]==0?'':' background-color: red; color: #fff;'" @click="change(index)">
						<view class="huadan">
							<text style="font-weight: 700;">{{item}}</text>
							<text style="margin-left: 5px; font-weight: 700;">{{props.fixSelectData[styledata.value.rate[index]]}}</text>
						</view>
...
<script setup>
	...
	const props =defineProps(['Refresh','fixSelectData','Tiemsort'])
	...
	const noshowgame=()=>{
		var value = props.fixSelectData.matchandstate
		if(value=='销售中'){
			return true
		}else{
			return false
