...
<script setup>
	...
	const dataProcessing=()=>{
		...
		var minValue=Math.min(...eachNote)//找到单注最小值
		var minValueIndex=eachNote.indexOf(minValue)//拿到最小值的index
		money.value=multiplying.value*2//方案金额
		var residueNote=null//已分布注数（为了方便后续算还剩下的可投注的数量）
