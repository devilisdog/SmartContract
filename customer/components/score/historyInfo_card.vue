	...
	<view class="card" v-if="show && Props.type=='lq'">
		...
		<view class="number_box">
			<view class="number_item">
				<text class="bigText">
					{{Number(Props.cardInfo.resultStat[0]/(Props.cardInfo.resultStat[0]+Props.cardInfo.resultStat[1])*100).toFixed(0)}}%
				</text><br>
				<text>胜率</text><br>
				<text class="minText">
			...
			<view class="number_item">
				<text class="bigText">
					{{Number(Props.cardInfo.letResultStat[0]/(Props.cardInfo.letResultStat[0]+Props.cardInfo.letResultStat[1]+Props.cardInfo.letResultStat[2])*100).toFixed(0)}}%
				</text><br>
				<text>赢盘率</text><br>
				<text class="minText">
					{{Props.cardInfo.letResultStat[0]}}胜{{Props.cardInfo.letResultStat[1]}}走{{Props.cardInfo.letResultStat[2]}}输
				</text>
			</view>
			<view class="number_item">
				<text class="bigText">
					{{Number(Props.cardInfo.bigsmallResultStat[0]/(Props.cardInfo.bigsmallResultStat[0]+Props.cardInfo.bigsmallResultStat[1]+Props.cardInfo.bigsmallResultStat[2])*100).toFixed(0)}}%
				</text><br>
				<text>大球率</text><br>
				<text class="minText">
					{{Props.cardInfo.bigsmallResultStat[0]}}大{{Props.cardInfo.bigsmallResultStat[1]}}走{{Props.cardInfo.bigsmallResultStat[2]}}小
				</text>
			</view>
		</view>
		<uni-table stripe emptyText="暂无更多数据" >
			...
			<uni-tr v-for="itme,index in Props.cardInfo.matchVos">
				<uni-td align="center">
