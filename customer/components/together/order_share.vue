		...
		<l-painter-view :css="shareStyle.top">
			<l-painter-view :css="shareStyle.top_user_info">
				<l-painter-view css="display:inline-block;">
					<l-painter-image :src="Props.order_info.content[0].user_id.avatar" :css="shareStyle.user_info_logo" />
					<l-painter-text :text="Props.order_info.content[0].user_id.nickname+'\n\n邀请您参与合买~'" css="margin-left:10rpx;line-height:45rpx;"></l-painter-text>
				</l-painter-view>
				<l-painter-view :css="shareStyle.info_label">
					<l-painter-text :text="'仅剩'+(Props.order_info.NumberShare-Props.order_info.togetherShare)+'份'"></l-painter-text>
				</l-painter-view>
			</l-painter-view>
			<l-painter-view :css="shareStyle.text_box">
				<l-painter-text :text="Props.order_info.Type+'\n合买彩种'"></l-painter-text>
			</l-painter-view>
			<l-painter-view :css="shareStyle.text_box">
				<l-painter-text :text="(Props.order_info.content[0].OneselftMoney / Props.order_info.content[0].MyselfShare).toFixed(2)+'\n单份金额(元)'"></l-painter-text>
			</l-painter-view>
			<l-painter-view :css="shareStyle.text_box">
				<l-painter-text :text="Props.order_info.BetMoney+'\n合买总额(元)'"></l-painter-text>
			</l-painter-view>
		</l-painter-view>
...
<script setup>
	...
	const shareStyle=reactive({
		...
		info_label:"background-color:#fff;height:60rpx;display:inline-block;border-radius:30rpx 0 0 30rpx;position:absolute;right:0rpx;top:30rpx;color:#ff5500;font-weight: bold;text-align: center;line-height:60rpx;padding:0rpx 20rpx;",
		text_box:"display:inline-block;text-align: center;font-weight: bold;font-size:32rpx;margin:0rpx 36rpx",
	...
	onBeforeMount(()=>{
		var newObj={
			heading:'jmzs',//判断是否是橘子二维码表示
			type:'together',//判断玩法类型(together==合买/follow==跟单)
			open_order_id:Props.order_info.order_id
		}
		// console.log(Props.order_info);
