		...
		methods: {
			...
			getactionanimation() {
				api.Getkaiji({
					...
					if (res.data.code == 1) {
						if (res.data.data != null && res.data.data.waite_time > 0 && res.data.data.layoutInfo
							.length > 0) {
							this.showbg = true
				...
				return new Promise((resolve, reject) => {
					uni.request({
						...
						success: (res) => {
							...
							this.apiurll = apiUrl
							useCounterStore().baseUrl = apiUrl
				...
				if (this.cuncuurl[this.currentTry] == undefined) {
					...
					uni.request({
						...
						success: (res) => {
							...
							useCounterStore().baseUrl = this.cuncuurl[this.currentTry]
							// uni.navigateTo({
							// 	url: '/pages/initPage'
							// })
							if (this.isToday(uni.getStorageSync('timeday'))) {
