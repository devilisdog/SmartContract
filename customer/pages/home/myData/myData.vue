<template>
    ...
    <view style="background-color: white">
        <view class="allStyle" style="border-bottom: 1rpx solid #f8f8f8" @click="imgsClick()">
            ...
            <image style="width: 100rpx; height: 100rpx; border-radius: 60rpx; margin-right: 30rpx" :src="counter.user_imgs" mode="aspectFill"> </image>
    ...
    <view style="background-color: white; margin-top: 30rpx">
        <view class="allStyle" style="border-bottom: 1rpx solid #f8f8f8" v-show="counter.userAllData.userinfo.RealName == 1">
        ...
        <view class="allStyle" @click="counter.userAllData.userinfo.RealName == 0 ? proxy.$refs.authenticationPopup.open() : ''">
    ...
    <uni-popup ref="rename">
        <view style="width: 550rpx; height: 320rpx; background-color: white; border-radius: 25rpx">
            ...
            <view style="display: flex; align-items: center; justify-content: center; width: 450rpx; height: 100rpx; margin: auto; margin-top: 20rpx">
        ...
        <view class="authenticationCard">
            ...
            <uni-forms border>
                <uni-forms-item label="真实姓名:">
                    <uni-easyinput :inputBorder="false" placeholder="请输入真实姓名" type="string" v-model="authenticationValue.realNmae" />
                </uni-forms-item>
                <uni-forms-item label="身份证号:">
                    <uni-easyinput :inputBorder="false" placeholder="请输入身份证号码" type="string" v-model="authenticationValue.user_id" />
                </uni-forms-item>
                <uni-forms-item label="证件照片:">
                    <view class="id-card-upload">
                        <view class="id-card-item" @click="chooseIdCardFront">
                            <image v-if="authenticationValue.idCardFront" :src="authenticationValue.idCardFront" mode="aspectFill" class="preview-image"></image>
                        ...
                        <!-- <view class="id-card-item" @click="chooseIdCardBack">
							<image v-if="authenticationValue.idCardBack" :src="authenticationValue.idCardBack" mode="aspectFill" class="preview-image"></image>
							<view v-else class="upload-placeholder">
								<text class="upload-icon">+</text>
            ...
            <view class="card_bottom">
                <view class="newButton" style="background-color: #aaaaaa; flex: 1" @click="proxy.$refs.authenticationPopup.close()"> 取消 </view>
                <view class="newButton" style="background-color: #202736; flex: 1; margin-left: 20rpx" @click="setAuthenticationPopup"> 确定 </view>
            </view>
            <view class="note_box">
                <text style="font-size: 32rpx">注意事项：</text><br />
                <text> 1、请认证填写相关信息，此信息将会直接影响投注,提现，充值等功能</text><br />
                <text>2、此实名认证后将无法修改实名，如填写错误请联系平台处理</text><br />
                <text>3、金额提现只能提现到认证人绑定的收款平台、无法自定义提现人信息</text>
                <text>4、请确保上传的身份证照片清晰、完整，且与填写的个人信息一致</text>
            </view>
        </view>
    </uni-popup>
    ...
    <uni-popup ref="setWord" type="center">
        <view class="setWord_card">
            ...
            <view class="card_bottom">
                <view class="newButton" style="background-color: #aaaaaa; flex: 0.7" @click="proxy.$refs.setWord.close()"> 取消 </view>
                <view class="newButton" style="background-color: #202736; flex: 1; margin-left: 20rpx" @click="setPassworde"> 确定 </view>
            </view>
        ...
        <view class="phoneCard">
            ...
            <uni-forms-item label="验证码" required label-position="top">
                <view class="codeBox">
                    ...
                    <view :class="typeof codeNumber == 'string' && isPhone(setPhoneData.phoneNumber) ? 'codeButton' : 'codeButtonFalse'" @click="getCode">
                        {{ codeNumber }}
                    </view>
                </view>
            </uni-forms-item>
            <view class="card_bottom">
                <view class="newButton" style="background-color: #aaaaaa; flex: 0.7" @click="setPhoneNumebr('close')"> 取消 </view>
                <view class="newButton" style="background-color: #202736; flex: 1; margin-left: 20rpx" @click="setPhoneNumebr('open')"> 确定 </view>
            </view>
        </view>
    </uni-popup>
</template>
...
const nickClick = () => {
    ...
    if (common.flition(blurVal.value, sensitiveWordsArray)) {
        uni.showModal({
            title: '注意',
            content: '名称内疑似包含违禁词，请检查后重新发布，违禁内容参考：' + common.replace(blurVal.value, '*', sensitiveWordsArray),
            confirmText: '确认',
            confirmColor: '#f04b49',
                ...
                uni.chooseImage({
                    ...
                    success(res) {
                        ...
                        if (res.tempFiles[0].size > 10485760) {
							...
							uni.uploadFile({
                                url: counter.baseUrl + '/api/new_user/idCardRecognition',
                                ...
                                success: async res => {
                                    ...
                                    if (newData.code == 1) {
										const {data}=newData
										authenticationValue.idCardFront=tempFilePaths
										authenticationValue.realNmae=data.name
										authenticationValue.user_id=data.id_number
                ...
                uni.chooseImage({
                    ...
                    success(res) {
                        if (res.tempFiles[0].size > 10485760) {
                            ...
                            authenticationValue.idCardBack = res.tempFilePaths[0]
                        }
                    },
                })
            } else if (res.tapIndex === 1) {
