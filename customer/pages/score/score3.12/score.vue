<template>
	<z-paging ref="cardList" v-model="gameData" @query="dataList" :auto-show-back-to-top='true'>
		...
		<view v-else>
			...
			<view v-else>
				...
				<listitem 
					v-for="itme,index in gameData" :cardInfo='itme' :key="itme.matchId" 
					type="lq" @onClick="clickGameCard($event,'lq')" 
				/>
			<!-- 列表 -->
			</view>
	...
	<uni-popup type="bottom" ref="screen" :mask-click="false">
		...
		<view class="popupCard">
			...
			<view class="popup_button" style="background-color: #e8e8e8;color:black;margin-top:40rpx;" @click="closePopup">
				取消
			</view>
		</view>
	</uni-popup>
...
<script setup>
	...
	const popupData=reactive([//比赛类型分类
		{
			title:'全部',
			value:''
		},
	...
	const datecurrent =ref(9)//日期选择器默认值
	...
	onShow(()=>{
		api.Gettoken().then((res)=>{
			if(res.data.code==1){
				...
				if(proxy.$refs.cardList){proxy.$refs.cardList.refresh()}
			}
	...
	const popupSelect=(index)=>{//筛选弹窗里面选择时触发
	...
	const clickGameCard=(gameId,type)=>{//点击比赛卡片触发
		uni.navigateTo({//跳转至比赛详情
			url:`/pages/score/score_ticai/gameParticulars?info_id=${gameId}&gameType=${type}`,
			animationType:'slide-in-bottom'
		})
	}
	const dataList=(pageNo,pageSize)=>{//刷新(比赛列表)
		if(current.value==1){
	...
	const footballList=(pageNo,pageSize)=>{//获取数据足球
		closeAutoData()
		var gameType='407'
		if(current.value=1){
		...
		if(zqcurrent.value !=0){
			uni.request({
				...
				timeout:10000,
				method:'GET',
				header:{'Ba-User-Token':apitoken.value,'Server':'1'},
				data:{
	...
	const getVidoLive=(arr)=>{//获取竞彩视频直播
		// uni.request({
	...
	const basketballLst=(pageNo,pageSize)=>{//获取数据篮球
		var gameType='406'
	...
	const autoGetInfo=()=>{//判断是否需要自动获取数据
		var autoArr=[]
		setTimeout(()=>{
			if(current.value==1){
	...
	const GetUnderwayData=(arr,name)=>{//获取实时数据定时访问
		uni.request({
			url:'https://play3.honghuohuo.vip/api/common.Api.index/MatchData',
			timeout:10000,
			method:'GET',
	...
	const refreshListData=(newData)=>{//更新列表数据
		var copyNewData=JSON.parse(JSON.stringify(newData))
		for(var id in copyNewData){//循环获取到比赛中最新的数据（对象） id=matchId
			for(var index=0;index<gameData.value.length;index++){//循环当天列表所有的数据
				if(gameData.value[index].matchId==id){//找到需要更新数据的比赛 用matchId找
					for(var key in copyNewData[id]){//找到后，循环新数据中的key按照key覆盖之前的数据
	...
	const onClickzqItem = (e) => {//tab子分类足球
	...
	function onClickdateItem(e){//日期选择
	...
	function getselectdatearr(selectDate,date){//分类选择器数据处理
		dategroupitems.length=0
		var copyDate=JSON.parse(JSON.stringify(date))
		...
		copyDate.forEach(function(item,index){
			if(new Date(item) && new Date(item).toString()!=='Invalid Date'){
				if(selectDate==item){datecurrent.value=index}
				...
				if(selectDate==item){datecurrent.value=index}
				dategroupitems.push({date:item,week:'',initData:item})
			}
		})
	}
</script>
