	...
	<view>
		...
		<view class="navigation">
			...
			<view style="height: 100rpx;display: flex;align-items: center;justify-content: center;">
				...
				<text style="color: #fff;font-size: 36rpx;">福彩3D</text>
			</view>
		</view>
		...
		<!-- 历史栏 -->
		<view style="width: 750rpx;margin-top: 10rpx;" v-if="acceptShow">
			<!-- 历史中奖号码部分 -->
			<uni-collapse>
		...
		<view>
			<uni-segmented-control :current="current" :values="items.data" @clickItem="onClickItem" styleType="text"
				activeColor="#ff5959"></uni-segmented-control>
			...
			if (res.data.code === 1) {
				...
				for (var i = 0; i <= res.data.data.data.length - 1; i++) {
					if (i == 0) {
						...
						wqsj.lotteryDrawResult = res.data.data.data[i].red
						wqsj.SaleEndtime=res.data.data.data[i].SaleEndtime
					} else {
						var arr = {
							code: res.data.data.data[i].code,
							lotteryDrawResult: res.data.data.data[i].red,
