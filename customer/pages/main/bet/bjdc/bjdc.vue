<template>
	...
	<view class="navigation">
		...
		<view style="height: 100rpx;display: flex;align-items: center;justify-content: center;">
			...
			<view @click="change">
				<text style="color: #484848;font-size: 36rpx;font-weight: bold;margin-left: 40rpx;">{{title}}</text>
				<uni-icons type="bottom" color="#bebebe" size="15"></uni-icons>
			</view>
			<text style="position: absolute;right: 30rpx;color: #3b3b3b;font-size: 30rpx;" 
	...
	<template style="padding-bottom:100rpx;"  v-for="item,index in DataList" :key="index">
		<view style="padding-bottom:100rpx;" v-show="index==tabsData[showData_index].businessDate">
			<view class="content_itme" v-for="item02,index02 in item" v-show="pitchShow(item02.comp)">
				<view class="card_left">
					<text style="font-weight: bold;color: #434343;">{{item02.issue_num}}</text>
					<text style="padding: 10rpx 0;font-weight: bold;color: #434343;">
						{{item02.comp.replace(/[^\u4E00-\u9FA5]/g,'')}}
					</text>
					<text style="color: #868686;">{{common.dateFormat(item02.match_time*1000,2)}}(截)</text>
				</view>
				<view class="card_right">
					<view style="display: flex;padding:0rpx 10rpx;">
						<view style="flex:1;">
							...
							<text v-show="play==0" :style="Number(item02.odds.spf.goal)>0?'color:red;':'color:green;'">
								{{item02.odds.spf.goal!='0'?'('+item02.odds.spf.goal+')':''}}
							</text>
						</view>
						<text style="width: 20%;flex:0.3;">VS</text>
						<text style="font-size: 30rpx;width: 40%;font-weight: bold;flex:1;">{{item02.away}}</text>
					</view>
					<!-- 混合选项那一栏数据  通过父向子传参fixSelectData实现不同比赛传入不同数据  父组件绑定触发事件-->
					<spfcheckbox
						v-if="play==0" :Refresh="forceRefresh" 
						:fixSelectData="item02"  ref="childRef" 
						@getSelectList="getChildrenSelectList($event,common.dateFormat(item02.match_time*1000,3))" 
						:count="count" 
					/>
					<sxdscheckbox 
...
<script setup>
	...
	const getData = () =>{//获取比赛数据并处理数据格式
		...
		DataList.value = []
		api.GetNewGameList({'type':3}).then(res=>{
			if(res.data.code==1){
	...
	const gameDataInit=(initData)=>{//数据初始化
		...
		for(var key in common.objectOrder(initData)){
			...
			initData[key].forEach((itme,index)=>{
				newArr.push(itme.issue_num)
				itme['odds']=JSON.parse(itme['odds'])
				var screenDataBoole=false
				for(var i=0;i<screenData.length;i++){
					if(screenData[i].title==itme.comp){screenDataBoole=true;break}
				}
				if(!screenDataBoole){
					var newOjb={
						title:itme.comp,
