<template>
		...
		<view class="content_box" v-for="itme,index in data.matchList">
			<view class="left">
				<view style="display:flex;flex-direction: column; margin: auto; text-align:center;">
					<text
						style="margin: 2rpx;">场次{{index+1}}</text>
					<text
			...
			<view class="right" style="display: flex;flex-direction: column;">
				...
				<view
					...
					<view style="width: 45%; font-size: 25rpx;font-weight: bolder;">
	...
	<fuiDialog :show="ShowTIPS" title="温馨提示" content="暂时没有数据,可以去看看其他玩法哦 ; )" :buttons="fuiButtons" :maskClosable='false' @click='fuiClick'></fuiDialog>

</template>

<script setup>
	import Footer from "@/components/main/bet/sfc/footer.vue"
	import sfccheckbox from "@/components/main/bet/sfc/sfc-checkbox.vue"
	...
	//温馨提示的两个按钮点击
	const fuiClick = (e) =>{if(e.index == 0){back()}else{back()}}
	// 投注内容 传到下一个页面的数据模板
