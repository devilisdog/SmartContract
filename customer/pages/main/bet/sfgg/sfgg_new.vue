<template>
	<view class="navigation">
		...
		<view
			style="height: 100rpx;display: flex;align-items: center;justify-content: center;background-color: #202736; ">
			<uni-icons color="#fff" type="back" @click="back()" size="21"
				style='position: absolute;left: 30rpx;'></uni-icons>
			<view>
		...
		<view style="padding-bottom:100rpx;" v-show="index==tabsData[showData_index].businessDate">
			<view class="content_itme" v-for="itme02,index02 in item">
				...
				<view class="card_right">
					<view
						style="display: flex;justify-content: center;align-items: center;text-align: center;margin-top:20rpx;">
						<view style="width: 45%;">
							<text style="font-size: 30rpx;font-weight:bold;">{{itme02.HostFull}}</text>
							<text style="font-size: 30rpx;margin-left:10rpx;"
								:style="Number(itme02.Handicap)>0?'color:red;':'color:green;'">{{itme02.Handicap}}</text>
						</view>
						<text style="width:10%;">VS</text>
						<text style="font-size: 30rpx;width: 45%;font-weight:bold;">{{itme02.GuestFull}}</text>
					</view>
					<sfggcheckbox :Refresh="forceRefresh" :count="count" ref="childRef" :fixSelectData="itme02"
						@getSelectList="
	...
	<fuiDialog :show="ShowTIPS" title="温馨提示" content="暂时没有数据,可以去看看其他玩法吧 ; )" :buttons="fuiButtons" :maskClosable='false'
		@click='back'>
	</fuiDialog>
	<!-- 底部导航 -->
...
<script setup>
	...
	const Getbjdcdata = () => { //获取比赛数据并处理数据格式
		...
		DataList.value = {}
		uni.request({ //获取比赛数据
			url: 'http://8.210.175.179:8383/api.SportDataV1/odds?type=sfgg&key=qhkj&secret=a4f71eaa25a00144c35eb0a457e636c',
			method: 'POST',
			success(res) {
				uni.hideLoading()
	...
	const getjiezhitime = (time) => {
		if (counter.gameAstrict.advance) {
			systime.value = counter.gameAstrict.advance
		}
