<template>
	...
	<view class="navigation">
		...
		<view style="height: 80rpx;display: flex;align-items: center;justify-content: center;">
			...
			<text style="color: #fff;font-size: 34rpx;">双色球</text>
		</view>
	</view>
	<view style="height: var(--status-bar-height);padding-top: 85rpx"></view>
		<view class="content">
			<view style="margin-top: 20rpx;">
				<!-- 往期数据折页 -->
				<uni-collapse ref="wqsjss" :border='false'>
					...
					<uni-collapse-item title-border="none" :border="false">
								...
								<view class="boxss" style="display: flex;flex-direction: row;">
									...
									<view class="box" style="display: flex;flex-direction: row;">
										<view class="topshuzhi"
											style="align-items: center;justify-content: center;display: flex;background:radial-gradient(#ff7373,#ff4f4f)"
											v-for="(item,index) in wqsj.red" :key="index">
											<text class="tt2"
												style="align-items: center;justify-content: center;display: flex;">{{item}}</text>
										</view>
										
										<view class="topshuzhi"
											style="align-items: center;justify-content: center;display: flex;background:radial-gradient(#6ea3ff,#1e8bff)"
											v-for="(item,index) in wqsj.blue" :key="index">
											<text class="tt2"
												style="align-items: center;justify-content: center;display: flex;">{{item}}</text>
										</view>
									...
									<view
										style="width: 140rpx;margin-left: 10rpx;center;justify-content: center;display: flex;">
						...
						<view class="content" style="display: flex;flex-direction: column;margin-left: -45rpx;">
							<view class="boxss" v-for="(list,index) in wqsj.list" :key="index" :is-shadow="false"
								spacing="0px" margin="0px" padding="0px"
								:style="index%2==0?'background-color: #d6f0ff;':'background-color: #ffffff;'"
								style="width: 750rpx;">
								<view class="box" style="margin-right: 5rpx;margin-left 10rpx;">
									<text style="color:#999999; ">{{list.code}}期</text>
								</view>
								<view class="box" style="display: flex;flex-direction: row;margin-left: 5rpx;">
									<view class="textshuzhi2"
										style="align-items: center;justify-content: center;display: flex;color: #ff4f4f;"
										v-for="(item,index) in list.red" :key="index">
										<text class="tt">{{item}}</text>
									</view>
									<view class="textshuzhi2"
										style="align-items: center;justify-content: center;display: flex;color: #1e8bff;"
										v-for="(item,index) in list.blue" :key="index">
										<text class="tt">{{item}}</text>
									</view>
								</view>
								<view
				...
				<view style="background-color: white;">
					<uni-segmented-control :current="current" :values="groupitems" style-type="text" activeColor="#f04b49" @clickItem="clickItem" />
				</view>
				<view v-show="current == 0">
					<!-- 机选定胆 -->
					<uni-card style="background-color: white; margin-top: 30rpx;height: 750rpx;" 
						spacing="0px" margin="0px" padding="10rpx" :border='false' shadow="0px 5rpx 5rpx rgba(229, 229, 229, 0.1)">
						<view class="left" style="margin-left: auto; display: flex; margin-left: 10rpx; margin-top: 10rpx;margin-bottom: 25rpx;">
							<view style="display: flex;margin-top: 10rpx;">
								<text style="font-size:20px;font-weight: bold;">机选定胆：</text>
							</view>
							<view
								v-if="data_jixuan.value.data.dan.red.length==0 && data_jixuan.value.data.dan.blue.length==0?true:false"
							...
							<view v-else @click="tanchu" class="right"
								...
								<view style="margin-right: auto;display:flex;"
									v-for="(item2,index) in data_jixuan.value.data.dan.red">
									<text class="dingdanwenzi"
										style="color: #ff4f4f; font-size:30rpx">{{ssqff(item2)}}</text>
								</view>
								<view style="margin-left: auto;display:flex;"
									v-for="(item2,index) in data_jixuan.value.data.dan.blue">
									<text class="dingdanwenzi"
										style="color: #1e8bff; font-size:30rpx">{{ssqff(item2)}}</text>
								</view>
							</view>
						</view>
						<view style="margin: auto;width: 710rpx;height: 1rpx;background-color: #f7f7f7;"></view>

						<view class="left" style="display: flex;flex-direction: row;margin-top: 30rpx;margin-left: 10rpx;">
						...
						<!-- 红球显示 -->
						<view v-if="forceRefresh" v-for="(item,i) in data_jixuan.value.list" class="dataJixian-list">
							<view class="textshuzhi"
								style="background:radial-gradient(#ff7373,#ff4f4f);"
								v-for="(text,index) in item.red" :key="text" >
								<text class="tt" style="">{{ssqff(text)}}</text>
							</view>
							<view class="textshuzhi"
								style="background:radial-gradient(#6ea3ff,#1e8bff);"
								v-for="(text,index) in item.blue" :key="text">
								<text class="tt" style="">{{ssqff(text)}}</text>
							</view>
						</view>
					</uni-card>
				...
				<view v-show="current == 1">
					<uni-card style="background-color: white; margin-top: 30rpx;height: 1050rpx;"
						spacing="0px" margin="0px" padding="10px" :border="false" shadow="0px 5rpx 5rpx rgba(229, 229, 229, 0.1)">
						<view class="left" style="display: flex;align-items: center;justify-content: space-between;margin: 20rpx 0;">
							<view style="display: flex;">
								<text style="font-size:20px;font-weight: bold; margin-left: 10rpx">
									选中号码两次设胆
								</text>
							</view>
							<view class="claer" type="black" @click="clear">
						...
						<view class="shuzhi">
							<view v-for="item,index in 33" :key="item" :style="boxstlye_red[index]==0?'background:radial-gradient(#ffeeee,#ffeeee);color: #ff4f4f;':'background:radial-gradient(#ff7373,#ff4f4f);color: #ffffff;'"
								@click="click(item,'red')" style="margin:15rpx 15rpx;border-radius:50rpx;display: flex;">
								<view v-if="boxstlye_red[index]!=2" 
									style="width:70rpx;height:70rpx;font-size:23rpx;line-height:70rpx;"
								>
									<text class="lanqiuwenzi">{{item<10?'0'+item:item}}</text>
								</view>
								<view style="text-align: center;;width:70rpx;height:70rpx;display: flex; flex-direction: column;"
									v-else>
									<text style="font-size:30rpx;font-weight:700;">
										{{item<10?'0'+item:item}}
									</text>
									<text style="font-size: 20rpx; font-weight: 700; margin-top:-15rpx;">
										胆
									</text>
								</view>
							</view>
						...
						<view class="shuzhi">
							<view  v-for="item,index in 16" :key="item" :style="boxstlye_blue[index]==0?'background:radial-gradient(#eaf1ff,#eaf1ff);color: #1e8bff;':'background:radial-gradient(#7dabff,#1e8bff);color: #ffffff;'"
								@click="click(item,'blue')" style="margin:15rpx 15rpx;border-radius:50rpx;display: flex;">
								<view 
									style="width:70rpx;height:70rpx;font-size:23rpx;align-items: center;justify-content: center;display: flex;">
									<text class="lanqiuwenzi">{{item<10?'0'+item:item}}</text>
								</view>
							</view>
						</view>
		...
		<uni-popup ref="tcpopup" :maskClick="false">
			<view style="background-color:#fff;border-radius:20rpx 20rpx 0 0;">
				<daletoouqingxuanze @close="tcpopupclose" :ssqIndex='1' :ballquantity="{'red':33,'blue':16}" />
			</view>
		</uni-popup>
...
<script setup>
		...
		api.GetWelfareKJ({
		    ...
		    if (res.data.code === 1) {
				for(var i=0;i<=res.data.data.data.length-1;i++){
					...
					wqsj.blue = res.data.data.data[i].blue
					wqsj.SaleEndtime=res.data.data.data[i].SaleEndtime
						...
						var arr = {
							...
							blue:res.data.data.data[i].blue,
				...
				nextTick(() => {proxy.$refs.wqsjss.resize()})
				getjiezhitime()
		    }else{
	...
	const initjixuan=(e,data)=>{
		...
		if(e==1){
			...
			for(var i=0;i<=num.value-1;i++){
				var reddata = common.quickSort(common.GetRandomArray(1,33,6))
				var bluedata =common.quickSort(common.GetRandomArray(1,16,1))
				var arr = {
					red:reddata,
					blue:bluedata
				}
				data_jixuan.value.list.push(arr)
			}
		}else if(e==2){
			data_jixuan.value.note = num.value
			data_jixuan.value.money = num.value*2
			for(var i=0;i<=num.value-1;i++){
				...
				for(;;){
					count+=1
					var reddata = common.quickSort(common.GetRandomArray(1,33,6-data.red.length))
					var bluedata =common.quickSort(common.GetRandomArray(1,16,1-data.blue.length))
					...
					if(list1.length==6 && list2.length==1){
						break
					}else{
						reddata=[]
						bluedata=[]
	...
	const localdata =reactive([{value:'一注',number:1},{value:'三注',number:3},{value:'五注',number:5}])
	const change = (e)=>{
		num.value = e
	...
	const tcpopupclose = (data)=>{	// 机选定胆返回
		if(data!=null){
			initjixuan(2,data)
	...
	const click = (item,type)=>{
		if(type=='red'){
			...
			if(boxstlye_red[item-1]==1){
				...
				// 判断是否不超过四个增加胆
				if(data_zixuan.value.data.dan.length<4){
					data_zixuan.value.data.dan.push(item)
			...
			if(boxstlye_red[item-1]==2){
				// 删除胆
				var index = data_zixuan.value.data.dan.findIndex(i =>
					i == item 
				)
				data_zixuan.value.data.dan.splice(index, 1)
				boxstlye_red[item-1] = 0
				isnote()
				return
			}
	...
	const isnote =()=>{
		...
		var dannum = data_zixuan.value.data.dan.length
		if( num_blue!=0 &&num_red+dannum>=6){
			data_zixuan.value.note = common.TakeMN(num_red, 6 - dannum)*common.TakeMN(num_blue, 1)
	...
	const messageToggle=()=>{
		...
		if(current.value==0){
				data_jixuan.value['list']=data_jixuan.value.list
				var jixuan_arr = {
					note:1,
					money:2,
					beilv:1,
		...
		if(current.value==1){
			...
			if(data_zixuan.value.note > 0 || counter.submitNewData.length > 0){
				if(data_zixuan.value.data.list.red.length+data_zixuan.value.data.dan.length >= 6 && data_zixuan.value.data.list.blue.length >= 1){
	...
	const addNumber =()=>{
		...
		if(current.value==1){
			if(data_zixuan.value.note > 0 || counter.submitNewData.length > 0){
				if(data_zixuan.value.data.list.red.length+data_zixuan.value.data.dan.length >= 5 && data_zixuan.value.data.list.blue.length>= 1){
					counter.submitData.push(data_zixuan.value)
					counter.submitNewData.push(data_zixuan.value)
					uni.showToast({
						icon:'none',
