<template>
	...
	<view style="width:100%;background-color: white;margin-top:10rpx;">
		<view class="switch">
			<text>是否启用</text>
			<switch :checked="userData.robot_enabled==1?true:false" @change="robotenabled($event)" color="#FDC830"/>
		</view>
		<view class="" v-if="userData.robot_enabled==1">
			<view class="switch" @click="goyongprop">
				<text>每日跟单数</text>
				<view style="display: flex;align-items: center;">
					<text style="font-size:26rpx;color:#848484">{{userData.robot_follow_qty}}</text>
					<uni-icons type="forward"></uni-icons>
				</view>
			</view>
			<view class="switch" @click="gojieprop">
				<text>投注倍数</text>
				<view style="display: flex;align-items: center;">
					<text style="font-size:26rpx;color:#848484">{{userData.robot_follow_multiple}}</text>
					<uni-icons type="forward"></uni-icons>
				</view>
			</view>
			<view class="switch">
				<text>是否跟单</text>
				<switch :checked="userData.robot_follow_enabled==1" @change="clickSetSwitchdaili($event)" color="#FDC830"/>
			</view>
			<view class="switch" @click="changpsd">
				<text>一键重置(登陆密码)</text>
				<view style="display: flex;align-items: center;">
					<uni-icons type="forward"></uni-icons>
				</view>
	...
	<uni-popup ref="goyongpopup" type="dialog">
		<uni-popup-dialog ref="inputClose"  mode="input" title="跟单数设置" :value="userData.robot_follow_qty"
							placeholder="请输入跟单数量!!!" @confirm="yongyes"  @close="yongclose"></uni-popup-dialog>
	</uni-popup>
	<uni-popup ref="gojiepopup">
		<uni-popup-dialog ref="inputClose"  mode="input" title="投注倍数默认值" :value="userData.robot_follow_multiple"
							placeholder="请输入跟单投注倍数!!!" @confirm="tianyes"  @close="tianclose"></uni-popup-dialog>
	</uni-popup>
	
	<uni-popup ref="alertDialogpsd" type="dialog">
	  <uni-popup-dialog type="warn" cancelText="关闭" confirmText="同意" title="通知" content="是否选择一键重置密码" @confirm="dialogpsdConfirm"
						@close="dialogpsdClose"></uni-popup-dialog>
	</uni-popup>
	<uni-popup ref="yeschangpsd" type="dialog">
	 <dialogpsd :psdword="psdwod" @close="changepsdclose" @yeschange="changepsdcyes"></dialogpsd>
	</uni-popup>
...
<script setup>
	...
	import mainapi from "@/common/vmeitime-http/mine/mine.js"
	import getdatetime from "@/common/getdatetime.js"
	import user from "@/common/vmeitime-http/user/user.js"
	import dialogpsd from "@/components/psd-change-dialog/psd-change-dialog.vue"
	...
	const robotmultiple=ref(0)
	const radio=ref([])
	const changpssd=ref('0')
	const userData = reactive({
		...
		settle_days:0,//结算天数
		rate:0,//佣金比例
		robot_follow_enabled:0,
		robot_enabled:0,
		robot_follow_multiple:500,
		robot_follow_qty:10
	})
	...
	const getUserInfo=(id)=>{
		...
		api.GetUserData({"id":id}).then((res) => {
			...
			userData.rate=res.data.data.Config.rate
			userData.robot_follow_enabled=res.data.data.Config.robot_follow_enabled
			userData.robot_enabled=res.data.data.Config.robot_enabled
			userData.robot_follow_multiple=res.data.data.Config.robot_follow_multiple
			userData.robot_follow_qty=res.data.data.Config.robot_follow_qty
			uni.hideLoading()
		})
	}
	const goyongprop=()=>{
	...
	const robotenabled=(e)=>{
		let arr={}
		if(e.detail.value){
			arr.key='robot_enabled'
			arr.value=1,
			arr.id=props.id
		}else{
			arr.key='robot_enabled'
	...
	const changpsd=()=>{
		proxy.$refs.alertDialogpsd.open()
	}
	const psdwod=ref(0)
	const dialogpsdConfirm =()=>{
		api.chongzhimima({"id":userData.id}).then((res)=>{
			if(res.statusCode==200){
				psdwod.value=res.data.data
				proxy.$refs.yeschangpsd.open()
			}
		})
		proxy.$refs.alertDialogpsd.close()
	}
	const changepsdclose=()=>{
			proxy.$refs.yeschangpsd.close()
	}
	const changepsdcyes=()=>{
		proxy.$refs.yeschangpsd.close()
	}
	const dialogpsdClose =()=>{
	}
	...
	// 设置是否代理
	const clickSetSwitchdaili= (state)=>{//底部开关
		let arr={}
		if(state.detail.value){
			arr.key='robot_follow_enabled'
			arr.value=1,
			arr.id=props.id
		}else{
			arr.key='robot_follow_enabled'
